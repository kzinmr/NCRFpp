...
class Data:
    ...
    def build_pretrain_emb(self):
        if self.word_emb_dir:
            print(
                "Load pretrained word embedding, norm: %s, dir: %s"
        ...
        if self.char_emb_dir:
            print(
                "Load pretrained char embedding, norm: %s, dir: %s"
        ...
        for idx in range(self.feature_num):
            if self.feature_emb_dirs[idx]:
                ...
                (
                    self.pretrain_feature_embeddings[idx],
    ...
    def write_decoded_results(self, predict_results, name):
        ...
        for idx in range(sent_num):
            if self.sentence_classification:
                fout.write(
                    " ".join(content_list[idx][0]) + "\t" + predict_results[idx] + "\n"
                )
            else:
                ...
                for idy in range(sent_length):
                    ...
                    fout.write(
                        content_list[idx][0][idy].encode("utf-8")
                        + " "
        ...
        fout.close()
        print(
            "Predict %s result has been written into file. %s" % (name, self.decode_dir)
        )
    ...
    def write_nbest_decoded_results(self, predict_results, pred_scores, name):
        ...
        fout.close()
        print(
            "Predict %s %s-best result has been written into file. %s"
    ...
    def read_config(self, config_file):
        ...
        the_item = "word_dropout"
        if the_item in config:
            self.HP_word_dropout = float(config[the_item])
        the_item = "lstm_layer"
        if the_item in config:
